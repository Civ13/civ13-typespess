...
class Recharger extends Component {
	...
	attack_by(prev, item, user) {
		if (has_component(item, "Tool") && item.c.Tool.can_use("Wrench")) {
			if (this.charging) {
				to_chat`<span class='notice'>Remove the charging item first!</span>`(
					user
				);
				return true;
			}
			...
			to_chat`<span class='notice'>You ${
				this.a.c.Tangible.anchored ? "attach" : "detach"
			} the ${this.a}.</span>`(user);
			item.c.Tool.used("Wrench");
			return true;
		}
		if (has_component(item, "Rechargeable") && this.a.c.Tangible.anchored) {
			if (!this.a.c.ApcPowered.powered) {
				to_chat`<span class='notice'>The ${this.a} blinks red as you try to insert the ${item}.</span>`(
					user
				);
				return true;
			}
			if (!item.c.Rechargeable.check_can_charge(user, this.a)) return true;
