...
class Grille extends Component {
		...
		if (damage_type == "brute") {
			if (damage_amount)
				new Sound(this.a.server, {
					path: "sound/effects/grillehit.ogg",
					volume: 0.8,
					vary: true,
				}).emit_from(this.a);
			else
	...
	attack_by(prev, item, user) {
		...
		!this.a.c.Tangible.anchored)
				...
				visible_message`<span class='notice'>${user} ${
					this.a.c.Tangible.anchored ? "fastens" : "unfastens"
				} the ${this.a}.</span>`.self`<span class='notice'>You ${
					this.a.c.Tangible.anchored
						? `fasten the ${this.a} to`
						: `unfasten the ${this.a} from`
			...
			visible_message`<span class='notice'>${user} rebuilds the broken grille.</span>`
				.self`<span class='notice'>You rebuild the broken grille</span>`.emit_from(
				user
			);
			item.c.Stack.use(1);
			this.broken = false;
			...
			if (item.c.Stack.amount < 2) {
				to_chat`<span class='warning'>You need at least two sheets of glass for that!</span>`(
					user
				);
				return;
			}
			to_chat`<span class='notice'>You start placing the window...</span>`(
				user
			);
			user.c.MobInventory.do_after({ delay: 2000, target: this.a }).then(
				(success) => {
					if (!success || !this.a.loc) return;
					for (let atom of this.a.crosses()) {
						if (has_component(atom, "Window")) return;
					}
					let window = new Atom(this.a.server, "window");
					window.loc = this.a.loc;
					window.c.Tangible.anchored = false;
					item.c.Stack.use(2);
					to_chat`<span class='notice'>You place the ${window} on the ${this.a}.</span>`(
						user
					);
				}
			);
	...
	deconstruct(prev) {
		...
		if (!this.a.c.Destructible.no_deconstruct) {
			var rods = new Atom(
				this.a.server,
				this.a.server.templates[this.rods_type]
			);
