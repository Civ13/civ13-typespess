...
class MobMovement extends Component {
	...
	update_walk() {
		...
		if (walk_dir & 12 && walk_dir & 3) {
			walk_dir &= this.last_axis;
		}
	...
	move(prev: any, dx: number, dy: number) {
		...
		if (!this.atom.c.SimpleMob) {
			//lets skip bodyparts if its a simple mob
			for (const tlimb of [
				"torso",
				"groin",
				"l_arm",
			...
			for (const i in this.atom.c.MobInventory.slots) {
				if (i) {
					this.atom.c.MobInventory.slots[i].update_icons();
				}
			}
		}
		return prev();
