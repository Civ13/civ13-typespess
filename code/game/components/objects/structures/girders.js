...
class Girder extends Component {
	...
	attack_by(prev, item, user) {
		if (has_component(item, "Tool")) {
			if (item.c.Tool.can_use("Screwdriver", user)) {
				if (this.state == "displaced") {
					...
					to_chat`<span class='notice'>You start disassembling the girder.</span>`(
						user
					);
					...
					to_chat`<span class='notice'>You start ${
						this.state == "reinf" ? "unsecuring" : "securing"
					} support struts...</span>`(user);
					user.c.MobInventory.do_after({
						delay: 4000 * item.c.Tool.toolspeed,
						target: this.a,
					}).then((success) => {
						if (!success || this.state != orig_state) return;
						to_chat`<span class='notice'>You ${
							this.state == "reinf" ? "unsecure" : "secure"
						} the support struts...</span>`(user);
		...
		!(this.a.base_loc && this.a.base_loc.turf)
				...
				to_chat`<span class='notice'>You start ${
					this.state == "displaced" ? "securing" : "unsecuring"
				} the girder...</span>`(user);
				user.c.MobInventory.do_after({
					delay: 4000 * item.c.Tool.toolspeed,
					target: this.a,
				}).then((success) => {
					...
					visible_message`<span class='notice'>${user} ${
						this.state == "displaced" ? "unsecures" : "secures"
					} the ${this.a}.</span>`.self`<span class='notice'>You ${
						this.state == "displaced"
							? `unsecure the ${this.a} from`
							: `secure the ${this.a} to`
					} the floor.</span>`.emit_from(user);
				});
				return true;
			} else if (
		...
		this.state == "reinf_struts"
				...
				user.c.MobInventory.do_after({
					...
					if (!success || this.state != "reinf_struts") return;
					to_chat`<span class='notice'>You remove the inner grille.</span>`(
						user
					);
		...
		} else if (this.state == "normal" && has_component(item, "MetalSheet")) {
			//TODO: False walls
			if (item.c.Stack.amount >= 2) {
				to_chat`<span class='notice'>You start adding plating...</span>`(user);
				user.c.MobInventory.do_after({ delay: 4000, target: this.a }).then(
					(success) => {
						if (!success) return;
				...
				to_chat`<span class='notice'>You need two sheets of metal to finish a wall!</span>`(
					user
				);
				return true;
			}
			...
			if (this.state == "reinf") {
				to_chat`<span class='notice'>You start finalizing the reinforced wall...</span>`(
					user
				);
				user.c.MobInventory.do_after({ delay: 5000, target: this.a }).then(
					(success) => {
						if (!success || this.state != "reinf") return;
						item.c.Stack.use(1);
						to_chat`<span class='notice'>You fully reinforce the wall.</span>`(
							user
						);
				...
				to_chat`<span class='notice'>You start reinforcing the girder...</span>`(
					user
				);
				user.c.MobInventory.do_after({ delay: 6000, target: this.a }).then(
					(success) => {
						if (!success || this.state != "normal") return;
						item.c.Stack.use(1);
						to_chat`<span class='notice'>You reinforce the girder.</span>`(
							user
						);
						this.state = "reinf";
					}
