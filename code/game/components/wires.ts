...
class MachineWires extends Component {
	constructor(atom: any, template: any) {
		...
		for (const def of group_def) {
			const wire_obj = Object.assign({}, def);
	...
	!user.c.Mob.can_read_panel(this.a, MachineWirePanel)
		...
		const panel = new MachineWirePanel(user.c.Mob.client, {
			...
			width: 350,
			height: 150 + this.wires.length * 30,
		});
...
class MachineWirePanel extends Panel {
	constructor(client: any, obj: Record<string,any>) {
		...
		this.on("open", () => {
			...
			this.send_message({
				wires: wires_obj,
				status_text: this.bound_atom.c.MachineWires.status_text,
			});
		});
		this.on("close", () => {
		...
		tool.c.Tool.can_use("Multitool", this.bound_mob)
				...
				new Sound(this.client.server, {
					...
					vary: true,
					volume: 0.2,
				}).emit_from(this.bound_atom);
				const wire = this.bound_atom.c.MachineWires.colors[msg.pulse];
				if (wire) {
					wire.pulse();
				}
			}
