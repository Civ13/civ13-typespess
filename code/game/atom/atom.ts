...
class Atom extends EventEmitter {
	...
	*/
		...
		if (template.components) {
			for (let i = 0; i < template.components.length; i++) {
				...
				const templateVars = template.vars && template.vars.components && template.vars.components[componentName] ? template.vars.components[componentName]: {};
  ...
  */
	...
	set x(newX) {
		...
		this[_changeloc](
			...
			this[_y],
			this[_z],
			this.dim.location(newX, this[_y], this[_z])
		);
	}
	...
	set y(newY) {
		...
		this[_changeloc](
			...
			newY,
			this[_z],
			this.dim.location(this[_x], newY, this[_z])
		);
	}
	...
	set z(newZ) {
		...
		this[_changeloc](
			...
			this[_y],
			newZ,
			this.dim.location(this[_x], this[_y], newZ)
		);
	}
	...
	set dim(newDim) {
		...
		this[_changeloc](
			...
			this[_y],
			this[_z],
			newDim.location(this[_x], this[_y], this[_z])
		);
	}
  ...
  */
	move(offsetx: number, offsety: number, reason: any) {
		...
		const move_splits = Math.ceil(
			Math.max(
				Math.abs(offsetx) / this[_bounds_width],
				Math.abs(offsety) / this[_bounds_height]
			)
		);
		...
		this.movement_granularity;
			...
			this[_changeloc](
				...
				newy,
				this[_z],
				this.dim.location(newx, newy, this[_z])
			);
			remaining_x -= step_x;
			remaining_y -= step_y;
		}
  ...
  * @param {string} reason
  */
	can_move_within(atom: { can_crosser_move_within: (arg0: any, arg1: any, arg2: any, arg3: any) => any; }, offsetx: any, offsety: any, reason: any) {
