...
class TransitDockingPort extends Component {
	constructor(atom, template) {
		...
		this.helper = null;
		this.a.once("moved", () => {
			this.helper = new Atom(
				this.a.server,
				{ components: ["TransitHelper"], vars: { dir: this.transit_dir } },
				this.a.x,
				this.a.y,
				this.a.z,
				this.a.dim
	...
	undocked() {
		// we want to leave the z-level empty
...
class Shuttle extends Component {
	...
	dock(target) {
		...
		if (!target || target == old_dock) return;
		let dock_atoms = new Set(
			[...this.a.c.Area.touching, ...this.a.c.Area.area_brushes, this.a].filter(
				(atom) => {
					if (atom == old_dock)
					// obviously we don't bring the old dock with us
						return false;
					if (atom.dim != this.a.dim)
					// this shouldn't happen ever but meh
						return false;
					if (
