...
class ChemHeater extends Component {
	...
	!user.c.Mob.can_read_panel(this.a, ChemHeaterPanel)
		...
		var panel = new ChemHeaterPanel(user.c.Mob.client, {
			title: `${this.a.name}`,
			width: 275,
			height: 400,
		});
	...
	attack_by(prev, item, user) {
		if (has_component(item, "OpenReagentContainer")) {
			...
			this.beaker = item;
			to_chat`<span class='notice'>You add the ${item} to the ${this.a}.</span>`(
				user
			);
			return true;
		}
...
class ChemHeaterPanel extends Panel {
	constructor(client, panel_props) {
		...
		this.target_temperature_changed = this.target_temperature_changed.bind(
			this
		);
	...
	opened() {
		...
		this.bound_atom.c.ChemHeater.on(
			"target_temperature_changed",
			this.target_temperature_changed
		);
		this.bound_atom.c.ChemHeater.on("enabled_changed", this.enabled_changed);
		this.target_temperature_changed(
			null,
			this.bound_atom.c.ChemHeater.target_temperature
		);
	...
	closed() {
		this.bound_atom.c.ChemHeater.removeListener(
			"beaker_changed",
			this.beaker_changed
		);
		this.bound_atom.c.ChemHeater.removeListener(
	...
	this.bound_atom.c.ChemHeater.beaker
			...
			this.bound_atom.c.ChemHeater.beaker.c.ReagentHolder.add(
				msg.dispense,
				this.bound_atom.c.ChemHeater.dispense_amount
			);
		}
		if (msg.eject) {
