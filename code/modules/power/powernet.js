const { chain_func, has_component } = require("./../../../typespess/index.js");
...
class Powernet {
	constructor(power_controller) {
		...
		this.cables.add = chain_func(this.cables.add, (prev, val) => {
			...
			if (enable_debug_colors) val.color = this.debug_color;
			return prev();
		});
		this.cables.delete = chain_func(this.cables.delete, (prev, val) => {
			if (val.c.Cable.powernet == this) val.c.Cable.powernet = null;
			return prev();
		});
		this.cables.clear = chain_func(this.cables.clear, (prev) => {
			for (let cable of this.cables)
				if (cable.c.Cable.powernet == this) cable.c.Cable.powernet = null;
			return prev();
		});
		this.nodes = new Set();
		...
		this.nodes.delete = chain_func(this.nodes.delete, (prev, val) => {
			if (val.c.PowerNode.powernet == this) val.c.PowerNode.powernet = null;
			return prev();
		});
	...
	merge(powernet) {
		...
		for (let cable of new_cables) this.cables.add(cable);
		for (let node of new_nodes) this.nodes.add(node);
	...
	reset(dt) {
		...
		for (let smes of this.nodes) {
			if (has_component(smes, "Smes")) smes.c.Smes.restore(dt);
