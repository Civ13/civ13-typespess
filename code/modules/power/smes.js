...
class Smes extends Component {
	constructor(atom, template) {
		...
		this.on("outputting_changed", () => {
			this.update_outputting_overlay();
		});
		this.on("inputting_changed", () => {
			this.update_inputting_overlay();
		});
		this.on("input_attempt_changed", () => {
			this.update_inputting_overlay();
		});
		this.on("charge_changed", () => {
			this.update_charge_overlay();
		});
		this.on("capacity_changed", () => {
			this.update_charge_overlay();
		});
		...
		for (let varname of to_send) {
			let type = "number";
			if (
				["input_attempt", "output_attempt", "inputting", "outputting"].includes(
					varname
				)
			)
				type = "boolean";
	...
	update_inputting_overlay() {
		if (this.inputting)
			this.a.overlays.smes_inputting = {
				icon_state: "[parent]-oc1",
				overlay_layer: 1,
			};
		else if (this.input_attempt)
			this.a.overlays.smes_inputting = {
				icon_state: "[parent]-oc0",
				overlay_layer: 1,
			};
		else this.a.overlays.smes_inputting = null;
	}
	update_charge_overlay() {
		let clevel = Math.min(
			Math.max(Math.ceil((4.001 * this.charge) / this.capacity), 0),
			5
		);
		let overlay;
		if (clevel > 0) overlay = `[parent]-og${clevel}`;
		else overlay = null;
		if (
			!(
	...
	!user.c.Mob.can_read_panel(this.a, SmesPanel)
		...
		var panel = new SmesPanel(user.c.Mob.client, {
			title: `${this.a.name}`,
			width: 340,
			height: 440,
		});
	...
	this.input_attempt
		...
		(surplus + this.terminal.c.PowerNode.powernet.smes_input_total) / dt;
			if (this.inputting) {
				...
				this.last_input_potential = Math.min(
					(this.capacity - this.charge) * 1000,
					this.input_level * dt
				);
		...
		if (this.did_balance) this.did_balance = false;
		else this.input_available = 0;
		...
		if (this.did_restore) this.did_restore = false;
		else this.output_used = 0;
	}
...
class SmesPanel extends Panel {
	...
	opened() {
		for (let update_event of this.update_events) {
			this.bound_atom.c.Smes.on(
				`${update_event.varname}_changed`,
				update_event.listener_function
			);
			update_event.listener_function(
				null,
				this.bound_atom.c.Smes[update_event.varname]
			);
		}
	}
	...
	closed() {
		for (let update_event of this.update_events) {
			this.bound_atom.c.Smes.removeListener(
				`${update_event.varname}_changed`,
				update_event.listener_function
			);
		}
	...
	message_handler(msg) {
		...
		if (msg.input_level != null) {
			this.bound_atom.c.Smes.input_level = Math.min(
				this.bound_atom.c.Smes.input_level_max,
				Math.max(0, +msg.input_level || 0)
			);
		}
		if (msg.output_level != null) {
			this.bound_atom.c.Smes.output_level = Math.min(
				this.bound_atom.c.Smes.output_level_max,
				Math.max(0, +msg.output_level || 0)
			);
		}
	}
}
