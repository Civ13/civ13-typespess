...
class RequiresAccess extends Component {
	...
	build_access_function() {
		...
		try {
			this[_access_function] = new Function(
				"access_checker",
				`return ${expression};`
			);
		} catch (e) {
			this[_access_function] = () => {
				return false;
			};
		}
	}
	...
	can_access(target) {
		...
		else access_checker = () => false; // no access at all! But we still want to let people in if the thing doesn't actually need any access
...
class HasAccess extends Component {
	...
	can_access(target) {
		if (!has_component(target, "RequiresAccess")) return true;
