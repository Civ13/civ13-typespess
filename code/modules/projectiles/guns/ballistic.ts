...
class BallisticGun extends Component {
	constructor(atom: any, template: any) {
		...
		this.a.c.Gun.update_icon = chain_func(
			this.a.c.Gun.update_icon,
			this.update_icon.bind(this)
		);
	...
	this.mag_form_factor === item.c.AmmoBox.form_factor
			...
			if (!item.slot || item.slot.can_unequip()) {
				if (this.magazine) {
					to_chat`<span class='notice'>You perform a tactical reload on the ${this.a}, replacing the magazine.</span>`(
						user
					);
					this.magazine.c.AmmoBox.update_icon();
					this.magazine.loc = this.a.base_mover.fine_loc;
				} else {
					to_chat`<span class='notice'>You insert the magazine into the ${this.a}.</span>`(
						user
					);
				}
				...
				to_chat`<span class='warning'>You cannot seem to get the ${this.a} out of your hands!</span>`(
					user
				);
				return true;
			}
	...
	attack_self(user: any) {
		if (this.magazine) {
			...
			this.magazine.c.AmmoBox.update_icon();
			this.magazine = null;
			to_chat`<span class='notice'>You pull the magazine out of the ${this.a}.</span>`(
				user
			);
		} else if (this.a.c.Gun.chambered) {
			this.a.c.Gun.chambered.loc = this.a.base_mover.fine_loc;
			this.a.c.Gun.chambered = null;
			to_chat`<span class='notice'>You unload the round from the ${this.a}'s chamber.</span>`(
				user
			);
		} else {
			to_chat`<span class='notice'>There's no magazine in the ${this.a}.</span>`(
				user
			);
		}
		this.a.c.Gun.update_icon();
