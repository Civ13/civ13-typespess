...
class EnergyGun extends Component {
	...
	update_charge_overlay() {
		...
		if (this.select_icon) {
			...
			this.a.overlays.energy_select_icon = {
				icon_state: `[parent]${select_suffix}`,
			};
		}
		if (cell_charge < shot.c.EnergyLens.e_cost) {
			...
			if (this.shaded_charge) {
				this.a.overlays.energy_charge = {
					icon_state: `[parent]_charge${ratio}`,
				};
			} else {
				for (let i = 0; i < this.charge_sections; i++) {
					if (i >= ratio) {
	...
	attack_self(user: any) {
		if (this.ammo_type.length > 1) {
			...
			if (shot && shot.c.EnergyLens.select_name) {
				to_chat`<span class='notice'>The ${this.a} is now set to ${shot.c.EnergyLens.select_name}.</span>`(
					user
				);
			}
		}
