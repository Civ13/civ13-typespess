...
if (global.is_bs_editor_env || global.is_test_env) {
	...
	if (server_config.gh_login.enabled) {
		global.Tserver.handle_login = function (ws: {
			...
			const message_handler = (msg: string) => {
				...
				if (obj.request_check === true) {
					database
						.authenticate(obj.name, obj.password)
	...
	if (server_config.https) {
		const proxies = {
			http: http.createServer(
				(
					...
					res: {writeHead: (arg0: number, arg1: {Location: string}) => void; end: () => void}
				) => {
					res.writeHead(301, {
		...
		net.createServer(
			(socket: {
				once: (arg0: string, arg1: (buffer: any) => void) => void;
				pause: () => void;
				unshift: (arg0: any) => void;
				resume: () => void;
			}) => {
