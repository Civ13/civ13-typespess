...
class TypespessClient extends EventEmitter {
	constructor(wsurl: string | undefined, resRoot = "") {
		super();
	...
	handle_login() {
		this.connection.send(
			JSON.stringify({ login: "guest" + Math.floor(Math.random() * 1000000) })
		);
		this.login_finish();
	}
...
TypespessClient.prototype.enqueue_icon_meta_load = require("./core/icon_loader.ts");
TypespessClient.prototype.anim_loop = require("./core/renderer.ts");
TypespessClient.prototype.get_audio_buffer = require("./core/audio_loader.ts");
