module.exports.ParallaxPlane = class ParallaxPlane extends Plane {
	no_click: boolean;
	parallax_velocity: number[];
	parallax_offset: number[];
	parallax_velocity_lasttimestamp: number;
		...
		for (let x = 0; x < 2; x++) {
			for (let y = 0; y < 2; y++) {
				for (let layer = 1; layer <= 2; layer++) {
					const parallax_atom : any = new Atom(this.client, {
	...
	composite_plane(eye_ctx: CanvasRenderingContext2D, timestamp: any) {
