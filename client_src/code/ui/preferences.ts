...
<input type="range" min="1" max="16" step="1" class="shadow-quality-slider" value=8>
	...
	handle_message(msg: { sprite_accessories: any; skin_tones: (skin_tones: any) => void; set_tab: any; char_prefs: { name: any; gender: string; age: any; skin_tone: any; hair_color: any; hair_style: string | number; }; name_valid: any; name_correction: any[]; job_preferences: any; job_metas: any; }) {
		...
		if (msg.char_prefs) {
			...
			if (msg.char_prefs.hair_color) {
				const hc = msg.char_prefs.hair_color;
				this.panel.$(
					".property-hair_color"
...
<div style='display:inline-block' class='job-pref-button-container'></div>`;
	...
	create_preview({
		...
		if (hair_style) {
			const hc = this.char_prefs.hair_color || [255, 255, 255];
