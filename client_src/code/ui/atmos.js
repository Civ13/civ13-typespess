...
class GasPumpPanel {
	constructor(panel) {
		...
		this.panel.$(".target_pressure").addEventListener("change", () => {
			this.panel.send_message({
				target_pressure: this.panel.$(".target_pressure").value ** 2 * 4500,
			});
		});
	}
	...
	handle_message(message) {
		if (message.turned_on != null) {
			if (message.turned_on) this.panel.$(".turned_on").classList.add("on");
			else this.panel.$(".turned_on").classList.remove("on");
		}
		if (message.target_pressure != null) {
			this.panel.$(".target_pressure").value = Math.sqrt(
				message.target_pressure / 4500
			);
			this.panel.$(
				".target_pressure_display"
			).textContent = message.target_pressure.toFixed(2);
		}
	}
}
