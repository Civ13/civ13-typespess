...
`;
	...
	handle_message(msg) {
		...
		if (msg.capacity != null || msg.charge != null) {
			if (msg.capacity != null) this.capacity = msg.capacity;
			if (msg.charge != null) this.charge = msg.charge;
			...
			if (charge_percent > 50) bar_elem.dataset.statusColor = "good";
			else if (charge_percent > 15) bar_elem.dataset.statusColor = "average";
			else bar_elem.dataset.statusColor = "bad";
		}
		if (msg.outputting != null || msg.charge != null) {
			if (msg.outputting != null) this.outputting = msg.outputting;
			...
			this.panel.$(".bar-outputting").dataset.statusColor = this.outputting
				? "good"
				: "bad";
		}
	...
	msg.output_used != null
			...
			if (msg.output_used != null) this.output_used = msg.output_used;
			...
			fills[0].style.width = `${
				(this.output_level * 100) / this.output_level_max
			}%`;
			fills[1].style.width = `${
				(this.output_used * 100) / this.output_level_max
			}%`;
		...
		if (msg.inputting != null || msg.charge != null || msg.capacity != null) {
			if (msg.inputting != null) this.inputting = msg.inputting;
			let elem = this.panel.$(".input-status");
			if (this.charge / this.capacity > 0.99) {
				elem.dataset.statusColor = "good";
			...
			this.panel.$(".bar-inputting").dataset.statusColor = this.inputting
				? "good"
				: "bad";
		}
	...
	msg.input_available != null
			...
			fills[0].style.width = `${
				(this.input_level * 100) / this.input_level_max
			}%`;
			fills[1].style.width = `${
				(this.input_available * 100) / this.input_level_max
			}%`;
