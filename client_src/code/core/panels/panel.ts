class Panel extends EventEmitter {
	constructor(
		...
		const uiframes : HTMLElement = document.getElementById("uiframes-container")!;
		this.uiframes = uiframes;
		uiframes.appendChild(this.container_obj);

		this.header_obj.addEventListener("mousedown", this._start_drag.bind(this));
		this.container_obj.addEventListener(
			"mousedown",
			this._start_resize.bind(this)
		);
		this.container_obj.addEventListener(
			"mousemove",
			this._container_mousemove.bind(this)
		);
		this.container_obj.addEventListener(
			"mouseout",
			this._container_mouseout.bind(this)
		);
		...
		if (can_close) {
			...
			this.close_button.addEventListener("mousedown", (e: { preventDefault: () => void; }) => {
				e.preventDefault();
			});
		}
	...
	send_message(message: { [x: number]: any; }) {
	...
	close() {
		...
		this.uiframes
			.removeChild(this.container_obj);
		this.emit("close");
	}

	click(e: { target: { closest: (arg0: string) => any; }; }) {
	...
	is_valid_button(elem: { classList: { contains: (arg0: string) => any; }; }) {
		return (
			elem &&
	elem.classList &&
